...
or fail, and how those outcomes change over time. It is PostgreSQL-backed and
server-side only. There is no external analytics vendor or public event-ingest
endpoint.
...
- Session linkage remains null until the authentication principal exposes a
  safe internal session ID without expanding token handling.
- Events cover the backend analysis endpoint only. Frontend section views and
  interactions are intentionally out of scope.
