...
The bounded single-VPS deployment procedure is documented in [Private Alpha deployment](../docs/private-alpha.md).
...
PostgreSQL ownership, pins, and active shares are absolute automated-retention protections. If protected artifacts consume the configured byte budget, new durable persistence is declined instead of evicting them; lowering the budget does not make an existing protected result unreadable. Storage failures do not corrupt a successful parser result, and ownership is assigned only after durable persistence succeeds.
...
Phase 2B applies authentication globally with explicit public metadata for health, CSRF bootstrap, registration, login, logout, and public share reads. Phase 3B enforces ownership for Recent, reopen, Compare, Trends, storage, private deletion/pinning, share management, and Batch preflight. Repeated user/hash pairs are idempotent, different users may own one deduplicated hash, and legacy artifacts remain unowned and privately invisible without inference or backfill. Public share capability reads remain independent of ownership. AnalyzeResult gzip artifacts, global Recent content metadata, and Share metadata remain filesystem-based. Physical reconciliation loads owned hashes in one PostgreSQL query and combines them with share protection; a failure in either protection source conservatively skips unknown deletion. Recent truncation cannot authorize deletion of owned data. Zero-owner/unshared artifacts remain eligible for bounded cleanup, and no scheduled GC is implemented. Backups need both PostgreSQL and artifact storage. Health remains HTTP 200 for disabled/healthy database states and returns HTTP 503 when an enabled database health check reports unavailable.
