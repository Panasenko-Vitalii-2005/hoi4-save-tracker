...
Phase 2B applies authentication globally with explicit public metadata for health, CSRF bootstrap, registration, login, logout, and public share reads. Phase 3A records ownership for successful authenticated analysis requests. Repeated user/hash pairs are idempotent, different users may own one deduplicated hash, and legacy artifacts remain unowned without inference or backfill. Analyzer, Recent, Compare, Trends, storage, reopen, and share-management APIs are not ownership-filtered until Phase 3B. Public share capability reads remain independent of ownership. AnalyzeResult gzip artifacts, Recent metadata, and Share metadata remain filesystem-based and behaviorally unchanged. Backups for a future SaaS deployment need both PostgreSQL and artifact storage.
...
The backend has resource/input hardening, default-private session authentication, double-submit CSRF enforcement, and durable ownership metadata for new analyses. It does not yet enforce ownership for reads or mutations and has no per-user isolation, distributed admission, or cross-process locking. Authenticated is not authorized until Phase 3B; treat it as a local/single-owner service until then.
